...
void main() {
  group('addRgbFrame', () {
    ...
    test('dithers a colour the table does not hold', () async {
      ...
      const side = 16;
      final rgb = Uint8List(side * side * 3)..fillRange(0, side * side * 3, 0x80);
      final sink = Collector();
      final gif = GifWriter(
        sink,
        width: side,
  ...
  group('the stream form accepts every frame shape', () {
    ...
    test('an indexed GifFrame still works, unchanged from 0.1.x', () async {
      final indices = Uint8List.fromList(<int>[for (var i = 0; i < 16; i++) i % 16]);
      final sink = Collector();
      await Stream<GifFrame>.fromIterable(<GifFrame>[
...
final Uint8List _indexedGolden = Uint8List.fromList(const <int>[
  71, 73, 70, 56, 57, 97, 8, 0, 8, 0, 243, 0, 0, 0, 0, 0, 17, 17, 17, 34, 34,
  34, 51, 51, 51, 68, 68, 68, 85, 85, 85, 102, 102, 102, 119, 119, 119, 136,
  136, 136, 153, 153, 153, 170, 170, 170, 187, 187, 187, 204, 204, 204, 221,
  221, 221, 238, 238, 238, 255, 255, 255, 33, 255, 11, 78, 69, 84, 83, 67, 65,
  80, 69, 50, 46, 48, 3, 1, 0, 0, 0, 33, 249, 4, 0, 5, 0, 0, 0, 44, 0, 0, 0, 0,
  8, 0, 8, 0, 0, 4, 28, 16, 4, 49, 72, 49, 7, 37, 181, 88, 115, 143, 68, 89,
  152, 198, 121, 160, 88, 93, 217, 214, 125, 225, 180, 150, 46, 250, 68, 0, 59,
]);
